...
const settingTargetBlocks = ["woocommerce/cart", "woocommerce/checkout"];

// Add attrs
function addAttributesToBlocks(settings, name) {
  if (typeof settings.attributes !== "undefined") {
    if (settingTargetBlocks.includes(name)) {
      settings.attributes = {
        ...settings.attributes,
  ...
  (BlockEdit) => {
    return (props) => {
      ...
      return (
        <Fragment>
          <BlockEdit {...props} />
          {isSelected && settingTargetBlocks.includes(props.name) && (
            <InspectorControls>
              <ToggleControl
...
const isSlotFillVisible =
  window.yayWholesaleMeta.is_requirement_visible ?? false;

const slotFillTargetBlocks = [
  "woocommerce/mini-cart-footer-block",
  "woocommerce/cart-order-summary-block",
  "woocommerce/checkout-order-summary-block",
];

const RequirementEdit = (props) => {
  const isNotCard = useMemo(
    () => props.name !== "woocommerce/cart-order-summary-block",
    [props.name]
  );
  return (
    <div className={isNotCard && "ywhs_requirement_section"}>
...
const RequirementEditSlotFill = wp.compose.createHigherOrderComponent(
  (BlockEdit) => (props) => {
    if (!slotFillTargetBlocks.includes(props.name)) {
      return <BlockEdit {...props} />;
    }

    if (!isSlotFillVisible) return <BlockEdit {...props} />;

    const isMiniCart = useMemo(
      () => props.name === "woocommerce/mini-cart-footer-block",
      [props.name]
    );

    return isMiniCart ? (
      <>
        <RequirementEdit {...props} />
        <BlockEdit {...props} />
      </>
    ) : (
      <>
...
export { addAttributesToBlocks, RequirementEditSlotFill };
