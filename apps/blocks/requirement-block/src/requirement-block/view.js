    ...
    callbacks: {
        ...
        async checkMetRequired() {
            ...
            if (isDiscounted) {
                ...
                let progressOfAmt = Math.min(100, actualSubtotal / Math.max(wholesaleRole.minOrderAmount, 1) * 100);
                let progressOfQty = Math.min(100, actualCount / Math.max(wholesaleRole.minOrderQuantity, 1) * 100);
