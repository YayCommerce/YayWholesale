...
class RegisterMenu {
	...
	public function __construct() {
		if ( ! defined( 'YAY_WHOLESALE_MENU_ORDER' ) ) {
			define( 'YAY_WHOLESALE_MENU_ORDER', 2 );
		}
		if ( ! defined( 'YAY_WHOLESALE_MENU_PRIORITY' ) ) {
			define( 'YAY_WHOLESALE_MENU_PRIORITY', 90 );
		}
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_yaycommerce_menu_scripts' ) );
		add_action( 'admin_menu', array( $this, 'settings_menu' ) );
		add_action( 'admin_menu', array( $this, 'add_placeholder_menu' ), YAY_WHOLESALE_MENU_PRIORITY + 1 );
	...
	public function get_submenus() {
		...
		if ( ! empty( $licensing_plugins_yay_mail ) || ! empty( $licensing_plugins_yay_pricing ) || ! empty( $licensing_plugins_yay_swatches ) || ! empty( $licensing_plugins_yay_extra ) || ! empty( $licensing_plugins_yay_smtp ) || ! empty( $yay_licensing_plugins ) ) {
			$submenus['yaycommerce-licenses'] = array(
				...
				'capability'         => 'manage_options',
				'render_callback'    => array( '\Yay_Wholesale\YayCommerceMenu\LicensesMenu', 'render' ),
				'load_data_callback' => array( '\Yay_Wholesale\YayCommerceMenu\LicensesMenu', 'load_data' ),
			);
		}
		...
		$submenus['yaycommerce-other-plugins'] = array(
			...
			'capability'         => 'manage_options',
			'render_callback'    => array( '\Yay_Wholesale\YayCommerceMenu\OtherPluginsMenu', 'render' ),
			'load_data_callback' => array( '\Yay_Wholesale\YayCommerceMenu\OtherPluginsMenu', 'load_data' ),
		);
