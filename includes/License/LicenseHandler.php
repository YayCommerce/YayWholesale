...
class LicenseHandler {
    ...
    <div class="error">     
        ...
        printf( esc_html__( '%1$s license key is required. %2$sPlease enter your license key to start using the plugin%3$s.', 'yay-wholesale-b2b' ), 'YayWholesaleB2B', '<a href="' . esc_url( admin_url( 'admin.php?page=yaycommerce-licenses' ) ) . '">', '</a>' );
        ?>
        </p>
    </div>
        <?php
    ...
    public function do_remove_license_request() {
        ...
        $licensing_plugin_slug = isset( $_POST[ CorePlugin::get( 'slug' ) . '_licensing_plugin' ] ) ? sanitize_text_field( wp_unslash( $_POST[ CorePlugin::get( 'slug' ) . '_licensing_plugin' ] ) ) : '';
